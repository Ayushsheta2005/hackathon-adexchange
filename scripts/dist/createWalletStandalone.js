...
function parseArgs(argv) {
    const args = { blockchain: "ARC-TESTNET", count: 1 };
    for (let i = 0; i < argv.length; i++) {
        ...
        switch (a) {
            case "--wallet-set-id":
                if (!next)
                    throw new Error("--wallet-set-id requires a value");
            ...
            case "--blockchain":
                if (!next)
                    throw new Error("--blockchain requires a value");
...
function loadCircleEnv() {
    ...
    if (!entitySecret || !/^[a-fA-F0-9]{64}$/.test(entitySecret)) {
        throw new Error("CIRCLE_ENTITY_SECRET is missing or not 64-char hex — generate one with `pnpm --filter @ade/scripts generate:entity-secret`.");
    }
...
//# sourceMappingURL=createWalletStandalone.js.map