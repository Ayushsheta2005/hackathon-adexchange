...
export async function runDepositGateway(deps = {}) {
    ...
    throw new Error(`Deposit not credited within ${config.DEPOSIT_TIMEOUT_MS}ms — Circle testnet typically credits in 13–19 min. Tx hash: ${submitted.depositTxHash ?? "(not reported)"}`);
}
function buildLiveClient(config) {
    ...
    const privateKey = config.BUYER_PRIVATE_KEY;
    return new GatewayClient({
        chain: config.BUYER_CHAIN,
        privateKey,
    });
}
function toUsdcAtomic(formatted) {
...
if (isEntry) {
    runDepositGateway().catch((err) => {
        console.error(err);
        process.exit(1);
    });
}
//# sourceMappingURL=depositGateway.js.map