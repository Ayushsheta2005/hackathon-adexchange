...
export async function runDemoLoad(deps = {}) {
    ...
    if (!BUYER_WALLET_ID || !BUYER_WALLET_ADDRESS || !SELLER_WALLET_ADDRESS) {
        throw new Error("Missing wallet env vars. demoLoad requires BUYER_WALLET_ID, BUYER_WALLET_ADDRESS, SELLER_WALLET_ADDRESS — run create:wallets and copy the banner into .env.local.");
    }
    ...
    const requiredAtomic = toAtomic(MAX_CLEARING_PRICE_USDC) * BigInt(cycles) + toAtomic(PREFLIGHT_HEADROOM_USDC);
    if (toAtomic(balance.usdc) < requiredAtomic) {
        throw new Error(`Buyer balance ${balance.usdc} USDC is below the ${fromAtomic(requiredAtomic)} USDC required for ${cycles} cycles. Run fund:wallets and retry.`);
    }
    ...
    if (results.length < cycles) {
        throw new Error(`Demo load produced ${results.length}/${cycles} transactions — hackathon gate not met`);
    }
    await writeManifest(results);
    const totalUsdcSettled = fromAtomic(totalAtomic);
    banner("Demo Load — complete", buildMarginExplainer({ cycles: results.length, totalUsdcSettled }));
 ...
 */
async function defaultWriteManifest(results) {
...
if (isEntry) {
    runDemoLoad().catch((err) => {
        console.error(err);
        process.exit(1);
    });
}
//# sourceMappingURL=demoLoad.js.map