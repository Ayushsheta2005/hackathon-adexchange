import { resolvePersonasFromEnv } from "@ade/server";
...
async function runOnce() {
    ...
    if (!health || !health.ok) {
        throw new Error(`Exchange not reachable at ${exchangeUrl}/health. Run: pnpm --filter @ade/server dev`);
    }
    const personas = resolvePersonasFromEnv(process.env);
    if (personas.length === 0) {
        throw new Error("No personas resolved. Set BUYER_LUXURYCO/GROWTHCO/RETAILCO_WALLET_ID + _ADDRESS in .env.local.");
    }
    const circle = createCircleClient({ env: process.env });
    const balances = await Promise.all(personas.map(async (p) => {
        try {
