...
const blankToUndefined = (schema) => z.preprocess((v) => (typeof v === "string" && v.trim() === "" ? undefined : v), schema);
/**
...
const ScriptsEnvSchema = z.object({
    ...
    BUYER_WALLET_ADDRESS: blankToUndefined(z
        .string()
        .regex(/^0x[a-fA-F0-9]{40}$/)
        .optional()),
    SELLER_WALLET_ID: blankToUndefined(z.string().min(1).optional()),
    SELLER_WALLET_ADDRESS: blankToUndefined(z
        .string()
        .regex(/^0x[a-fA-F0-9]{40}$/)
        .optional()),
    MARKETPLACE_WALLET_ADDRESS: blankToUndefined(z
        .string()
        .regex(/^0x[a-fA-F0-9]{40}$/)
        .optional()),
    DEPOSIT_AMOUNT_USDC: blankToUndefined(z
        .string()
        .regex(/^\d+(?:\.\d{1,6})?$/)
        .default("0.10")),
    ...
    BUYER_PRIVATE_KEY: blankToUndefined(z
        .string()
    ...
    BUYER_LUXURYCO_WALLET_ADDRESS: blankToUndefined(z
        .string()
        .regex(/^0x[a-fA-F0-9]{40}$/)
        .optional()),
    BUYER_GROWTHCO_WALLET_ID: blankToUndefined(z.string().min(1).optional()),
    BUYER_GROWTHCO_WALLET_ADDRESS: blankToUndefined(z
        .string()
        .regex(/^0x[a-fA-F0-9]{40}$/)
        .optional()),
    BUYER_RETAILCO_WALLET_ID: blankToUndefined(z.string().min(1).optional()),
    BUYER_RETAILCO_WALLET_ADDRESS: blankToUndefined(z
        .string()
        .regex(/^0x[a-fA-F0-9]{40}$/)
        .optional()),
});
export function loadScriptsConfig(env = process.env) {
    loadRootEnv();
    const parsed = ScriptsEnvSchema.safeParse(env);
    if (!parsed.success) {
        throw new Error(`Invalid scripts env: ${parsed.error.issues
...
export function assertTestnet(config) {
    if (config.CIRCLE_ENVIRONMENT !== "testnet") {
        const confirm = process.env.CONFIRM_MAINNET;
        if (confirm !== "yes") {
            throw new Error("Refusing to run: CIRCLE_ENVIRONMENT is not 'testnet'. Set CONFIRM_MAINNET=yes to override.");
        }
    }
}
