...
import { ARC_TESTNET_EXPLORER_BASE, BidRequestSchema, MAX_CLEARING_PRICE_USDC, NANOPAYMENT_UNIT_USDC, } from "@ade/shared";
 ...
 */
export async function runDemoCycle(deps) {
    ...
    if (!auctionRes.ok) {
        ...
        throw new Error(`POST /auction/run/${deps.listingId} failed: ${auctionRes.status} ${JSON.stringify(body)}`);
    }
    const { auctionResult, receipt } = (await auctionRes.json());
 ...
 */
export function pickBidAmount(floorUsdc, rand) {
    ...
    if (minAtomic >= capAtomic)
        return fromAtomic(capAtomic);
...
//# sourceMappingURL=demoLoad.cycle.js.map