...
export const NEXUS_CHIP_SUGGESTIONS = [
...
export type NexusChipSuggestion = (typeof NEXUS_CHIP_SUGGESTIONS)[number];

export function isNexusChipSuggestion(text: string): text is NexusChipSuggestion {
  return (NEXUS_CHIP_SUGGESTIONS as readonly string[]).includes(text);
}

export interface NexusChipDemoPayload {
  reply: string;
  blocks: AssistantUiBlock[];
}

export function getNexusChipDemoReply(text: string): NexusChipDemoPayload | null {
  if (!isNexusChipSuggestion(text)) {
    return null;
  }
  switch (text) {
    case NEXUS_CHIP_SUGGESTIONS[0]:
      return {
        reply:
