/** Composer-only modes for Nexus trader input (not persisted). */
export type NexusComposerMode = "direct" | "goal" | "policy";

/** Canned assistant replies for composer sends (demo UX). */
export function getSimulatedNexusReply(mode: NexusComposerMode, message: string): string {
  if (mode === "goal") {
  ...
  if (/\b(bid|bids|bidding|cpm)\b/i.test(message)) {
    return "Got it. Executing bid strategy on YouTube CTV for Nike 1P with $20k daily cap through Sunday.";
  }
  if (/\bpaus(e|ed|es|ing)?\b/i.test(message)) {
    return "Pausing that supply path now. Budget will be redistributed to Netflix and Prime Video.";
  }
