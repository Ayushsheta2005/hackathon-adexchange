...
function NavRow({ label, badge }: { label: string; badge?: number }): JSX.Element {
  return (
    ...
    >
      ...
      {badge != null && badge > 0 ? (
        <span className="flex h-5 min-w-[1.25rem] items-center justify-center rounded-full bg-amber-500 px-1 font-nexus-mono text-[9px] font-bold text-white">
          {badge}
        </span>
      ) : null}
    </button>
    ...
    <>
      ...
      <div className="mt-2 border-t border-[oklch(0.94_0.004_80)] px-3 py-3">
        <div className="rounded-lg border border-[oklch(0.91_0.005_80)] bg-white/60 px-3 py-2.5 shadow-sm">
          <div className="text-[12px] font-semibold text-[oklch(0.22_0.01_80)]">Prime Video</div>
          <div className="mt-0.5 truncate font-nexus-mono text-[10.5px] text-[oklch(0.5_0.01_80)]">
            prime_videodaily.com
          </div>
        </div>
      </div>
