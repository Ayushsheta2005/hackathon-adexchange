...
export function NexusAnalyzeMode({
  period,
  onPeriod,
  applyNext,
  onApplyNext,
}: NexusAnalyzeModeProps): JSX.Element {
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap gap-1">
        {PERIODS.map((p) => (
          <button
            ...
            className={`rounded-full border px-3 py-1.5 font-nexus-mono text-[10.5px] font-semibold uppercase tracking-wide ${
              period === p.id
      ...
      <div className="rounded-[10px] border border-[oklch(0.91_0.005_80)] bg-white p-4 shadow-sm">
        <div className="font-nexus-mono text-[9.5px] font-semibold uppercase tracking-wider text-[oklch(0.55_0.006_80)]">
          Executive summary
        </div>
        <p className="mt-2 text-[13px] leading-relaxed text-[oklch(0.22_0.01_80)]">
          Nexus ran 14h 22m · 63.1% win rate · $184k settled · +$1,284 vs benchmark
        </p>
      </div>

      <div className="overflow-hidden rounded-[10px] border border-[oklch(0.91_0.005_80)] bg-white shadow-sm">
        <div className="border-b border-[oklch(0.94_0.004_80)] px-3 py-2 font-nexus-mono text-[9.5px] font-semibold uppercase tracking-wider text-[oklch(0.55_0.006_80)]">
          By channel
        </div>
        <div className="overflow-x-auto">
          <table className="w-full min-w-[320px] border-collapse text-left text-[12px]">
            <thead>
              <tr className="border-b border-[oklch(0.94_0.004_80)] font-nexus-mono text-[9.5px] uppercase tracking-wider text-[oklch(0.55_0.006_80)]">
            ...
            <tbody>
              {MOCK_ANALYZE_ROWS.map((row) => (
                ...
                >
                  ...
                  <td className="px-2 py-2 font-nexus-mono text-[oklch(0.32_0.01_80)]">
                    {row.imps}
                  </td>
                  <td className="px-2 py-2 font-nexus-mono">{row.winPct}</td>
                  <td className="px-2 py-2 font-nexus-mono">{row.vcr}</td>
                  <td className="px-2 py-2 font-nexus-mono">{row.ecpm}</td>
                  <td className="px-3 py-2 font-nexus-mono font-medium">{row.vsGoal}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      <div>
        <div className="mb-2 font-nexus-mono text-[9.5px] font-semibold uppercase tracking-wider text-[oklch(0.55_0.006_80)]">
          Top Nexus decisions
        </div>
        <div className="grid gap-2 sm:grid-cols-3">
          <div className="rounded-[10px] border border-[oklch(0.91_0.005_80)] border-l-[3px] border-l-emerald-500 bg-white p-3 shadow-sm">
            <div className="font-nexus-mono text-[9px] font-semibold uppercase text-emerald-800">
              Best call
            </div>
            <p className="mt-2 text-[11.5px] leading-snug text-[oklch(0.28_0.01_80)]">
              Held bid floor on Prime Video when VCR was already above goal.
            </p>
          </div>
          <div className="rounded-[10px] border border-[oklch(0.91_0.005_80)] border-l-[3px] border-l-amber-500 bg-white p-3 shadow-sm">
            <div className="font-nexus-mono text-[9px] font-semibold uppercase text-amber-900">
              Close call
            </div>
            <p className="mt-2 text-[11.5px] leading-snug text-[oklch(0.28_0.01_80)]">
              Almost shifted budget to Disney+; paused when win-rate dipped.
            </p>
          </div>
          <div className="rounded-[10px] border border-[oklch(0.91_0.005_80)] border-l-[3px] border-l-sky-500 bg-white p-3 shadow-sm">
            <div className="font-nexus-mono text-[9px] font-semibold uppercase text-sky-900">
              Learned
            </div>
            <p className="mt-2 text-[11.5px] leading-snug text-[oklch(0.28_0.01_80)]">
              YouTube dayparts after 9pm outperform for Nike 1P.
            </p>
          </div>
        </div>
      </div>
