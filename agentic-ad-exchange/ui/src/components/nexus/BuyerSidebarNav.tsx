...
export function BuyerSidebarNav(): JSX.Element {
  ...
  const nexusActive = pathname === "/buyer" || pathname === "/nexus";
  ...
  return (
    <>
      <nav className="flex flex-col gap-0.5 p-3" aria-label="Workspace">
        ...
        >
          <span
            className={`h-1.5 w-1.5 rounded-full ${nexusActive ? "bg-[#53d2dc]" : "bg-[oklch(0.72_0.006_80)]"}`}
          />
          Nexus
        </NavLink>
      </nav>
      <div className="flex min-h-0 flex-1 flex-col gap-2 overflow-y-auto px-3 pb-2 pt-1">
        <div className="flex items-baseline justify-between px-1">
          <span className="font-nexus-mono text-[10px] font-medium uppercase tracking-[0.08em] text-[oklch(0.55_0.006_80)]">
            Topics
          </span>
          <span className="font-nexus-mono text-[10px] text-[oklch(0.62_0.006_80)]">soon</span>
        </div>
        <ul
          className="list-none flex flex-col gap-1 p-0"
          aria-label="Nexus context shortcuts (placeholders)"
        >
