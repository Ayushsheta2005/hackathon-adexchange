...
import {
  NEXUS_CHIP_SUGGESTIONS,
  getNexusChipDemoReply,
} from "../assistant/nexusChipDemoCatalog.js";
import {
  getSimulatedNexusReply,
  type NexusComposerMode,
} from "../assistant/nexusComposerSimulation.js";
import {
...
export interface UseBuyerAssistantChatResult {
  ...
  sendComposerMessage: (userText: string, mode: NexusComposerMode) => void;
...
export function useBuyerAssistantChat(): UseBuyerAssistantChatResult {
  ...
  const [messages, setMessages] = useState<ChatLine[]>(() => [
    {
      ...
      role: "assistant",
      content:
        "I'm **Nexus**, your in-app assistant for the Agentic Ad Exchange. **Suggestion chips** insert **local demo replies** when they match a preset, or call the **live assistant** (server + Gemini when configured). Messages you **type below** use a **local simulation** (typing indicator + canned replies) for this demo.",
  ...
  const sendWithHistory = useCallback(
    async (userText: string) => {
      ...
      const chipDemo = getNexusChipDemoReply(userText);
      if (chipDemo !== null) {
  ...
  const sendComposerMessage = useCallback(
    (userText: string, mode: NexusComposerMode) => {
      ...
      composerSimTimerRef.current = setTimeout(() => {
        composerSimTimerRef.current = null;
        const reply = getSimulatedNexusReply(mode, userText);
        const withAssistant: ChatLine[] = [
          ...messagesRef.current,
          {
            id: newId(),
  ...
  return {
    ...
    chipSuggestions: NEXUS_CHIP_SUGGESTIONS,
  };
}
