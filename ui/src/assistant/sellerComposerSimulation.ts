...
export function getSimulatedSellerReply(mode: SellerComposerMode, message: string): string {
  ...
  if (mode === "set_floor") {
    ...
    return "Floor update noted. Nexus will apply this to the next auction cycle.";
  }

  if (mode === "configure_deal") {
    return "Deal structured as draft: HBO Max · $4.80 CPM · 30d · 8.4M imps/mo. Review in the Deals tab before activating.";
  }
  ...
  return "Revenue dropped 18% yesterday on CTV pre-roll due to floor at $4.20 cutting demand from 3 DSPs. Netflix and Prime Video compensated partially. Recommend floor review.";
}
