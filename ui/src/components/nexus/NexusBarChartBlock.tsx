...
export function NexusBarChartBlock({ block }: { block: AssistantBarChartBlock }): JSX.Element {
        ...
        <div className="flex flex-wrap items-start justify-between gap-2">
          ...
          <div className="flex shrink-0 flex-col items-end gap-1">
            {block.dataSource === "simulated" ? (
              <span
                className="rounded bg-amber-50 px-1.5 py-0.5 font-nexus-mono text-[8px] font-medium uppercase text-amber-900"
                title="Illustrative demo series"
              >
                sim
              </span>
            ) : (
              <span className="rounded border border-[oklch(0.91_0.005_80)] bg-[oklch(0.97_0.005_80)] px-1.5 py-0.5 font-nexus-mono text-[8px] font-medium uppercase text-[oklch(0.42_0.01_80)]">
                exchange
              </span>
            )}
            {block.yCaption != null && block.yCaption.length > 0 ? (
              <span className="font-nexus-mono text-[9px] text-[oklch(0.55_0.006_80)]">
                {block.yCaption}
              </span>
            ) : null}
          </div>
            ...
            {block.points.map((p, i) => {
              ...
              return (
                ...
                >
                  ...
                  />
                  <span className="line-clamp-2 w-full text-center font-nexus-mono text-[9px] leading-tight text-[oklch(0.42_0.01_80)]">
                    {p.label}
                  </span>
                </div>
              );
