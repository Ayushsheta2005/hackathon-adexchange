...
import { NexusBarChartBlock } from "./NexusBarChartBlock.js";
import { renderBoldMarkdown } from "./inlineBold.js";

export interface NexusAssistantBlocksProps {
  blocks: AssistantUiBlock[];
}

export function NexusAssistantBlocks({ blocks }: NexusAssistantBlocksProps): JSX.Element {
  return (
...
function renderBlock(block: AssistantUiBlock): JSX.Element {
  switch (block.type) {
    ...
    case "bar_chart":
      return <NexusBarChartBlock block={block} />;
  }
}
        ...
        {block.items.map((item, i) => (
          ...
          >
            <div className="flex items-center gap-1.5">
              <span className="font-nexus-mono text-[9px] font-semibold uppercase tracking-wider text-[oklch(0.55_0.006_80)]">
                {item.label}
              </span>
              {item.dataSource === "simulated" ? (
                <span
                  className="rounded bg-amber-50 px-1 py-0 font-nexus-mono text-[8px] font-medium uppercase text-amber-900"
            ...
            <p className="mt-0.5 font-nexus-mono text-[13px] font-semibold text-[oklch(0.18_0.01_80)]">
              {item.value}
            </p>
          </div>
        ))}
...
function PillGroupBlock({
  ...
  return (
    <div className="rounded-[10px] border border-[oklch(0.91_0.005_80)] bg-white px-3 py-2.5 shadow-sm">
      <div className="font-nexus-mono text-[9px] font-semibold uppercase tracking-wider text-[oklch(0.55_0.006_80)]">
        {block.sectionTitle}
      </div>
      <div className="mt-1.5 flex flex-wrap gap-1.5">
        {block.pills.map((p, i) => (
          <span
            key={i}
            className={`inline-flex max-w-full rounded-full border px-2 py-0.5 font-nexus-mono text-[11px] leading-snug ${pillClass(p.variant)}`}
          >
            {p.text}
          </span>
        ))}
  ...
  return (
    <div className="overflow-hidden rounded-[10px] border border-[oklch(0.91_0.005_80)] bg-white shadow-sm">
      <div className="flex flex-wrap items-start justify-between gap-2 border-b border-[oklch(0.94_0.004_80)] px-3 py-2">
        ...
        {block.badge ? (
          <span className="shrink-0 rounded-full border border-[oklch(0.91_0.005_80)] bg-[oklch(0.97_0.005_80)] px-2 py-0.5 font-nexus-mono text-[10px] text-[oklch(0.4_0.01_80)]">
            {block.badge}
          </span>
        ) : null}
      </div>
      <div className="space-y-2 px-3 py-2.5 text-[13px] leading-relaxed text-[oklch(0.28_0.01_80)]">
        ...
        {block.reasoning.length > 0 ? (
          <div>
            <div className="mb-1 font-nexus-mono text-[9px] font-semibold uppercase tracking-wider text-[oklch(0.55_0.006_80)]">
              Reasoning
            </div>
