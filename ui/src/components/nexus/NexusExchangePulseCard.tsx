...
export interface NexusExchangePulseCardProps {
  connected: boolean;
  paused: boolean;
...
export function NexusExchangePulseCard({
  connected,
  paused,
  settlementCount,
  bidCount,
  ...
  lastAuction,
  lastReceipt,
}: NexusExchangePulseCardProps): JSX.Element {
  return (
    <div className="overflow-hidden rounded-[10px] border border-[oklch(0.91_0.005_80)] bg-white shadow-sm">
      ...
      <div className="p-4">
        <h3 className="mb-2 font-nexus-mono text-[9.5px] font-semibold uppercase tracking-wider text-[oklch(0.55_0.006_80)]">
          Listings
        </h3>
        ...
        {lastReceipt && (
          <div className="mt-3 rounded-lg border border-[oklch(0.94_0.004_80)] p-3">
            ...
            <p className="mt-1 font-nexus-mono text-xs text-[oklch(0.32_0.01_80)]">
              {lastReceipt.status}
...
function Vital({ label, value, ok }: { label: string; value: string; ok?: boolean }): JSX.Element {
  return (
    <div className="border-r border-[oklch(0.94_0.004_80)] px-3 py-3 last:border-r-0">
      ...
      <div
        className={`mt-1 font-nexus-mono text-sm font-semibold ${
          ok === true
            ? "text-emerald-700"
            : ok === false
              ? "text-amber-800"
