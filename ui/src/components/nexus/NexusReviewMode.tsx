...
export interface NexusReviewModeProps {
...
export function NexusReviewMode({
  pending,
  resolved,
  expandedId,
  onExpandedId,
  ...
  onResolve,
  autoExecutedToday,
}: NexusReviewModeProps): JSX.Element {
  if (pending.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center rounded-[10px] border border-dashed border-[oklch(0.91_0.005_80)] bg-[oklch(0.995_0.003_80)] px-6 py-14 text-center">
        ...
        <p className="mt-2 max-w-xs text-[12.5px] text-[oklch(0.5_0.01_80)]">
          Nexus will notify you when a decision needs your input
        </p>
      </div>
    );
  }
    ...
    <div className="space-y-4">
      <div>
        <div className="flex flex-wrap items-baseline gap-2">
          ...
          <span className="rounded-full bg-amber-100 px-2 py-0.5 font-nexus-mono text-[10px] font-semibold uppercase tracking-wide text-amber-900">
      ...
      <details className="rounded-[10px] border border-[oklch(0.91_0.005_80)] bg-white shadow-sm">
        <summary className="cursor-pointer px-4 py-3 font-nexus-mono text-[10px] font-semibold uppercase tracking-wider text-[oklch(0.55_0.006_80)]">
          Recent · last 24h
        </summary>
        <ul className="list-none space-y-2 border-t border-[oklch(0.94_0.004_80)] p-3">
          {resolved.map((r) => (
            <li
              key={r.id}
              className="flex items-center gap-2 font-nexus-mono text-[11.5px] text-[oklch(0.32_0.01_80)]"
            >
