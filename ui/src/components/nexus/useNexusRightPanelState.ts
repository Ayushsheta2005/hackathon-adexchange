...
import type {
  ...
  ResolvedDecision,
  ReviewDecision,
} from "./nexusRightPanelTypes.js";
import { parseObjectiveToChips } from "./parseObjectiveChips.js";

const OBJECTIVE_PREFILL =
  "Maximize VCR for Nike 1P audience on CTV, $50k weekly cap, run through Sunday";

export interface UseNexusRightPanelStateResult {
  activeMode: NexusPanelMode;
  setActiveMode: (m: NexusPanelMode) => void;
  createStep: 1 | 2 | 3;
...
export function useNexusRightPanelState(): UseNexusRightPanelStateResult {
  const [activeMode, setActiveMode] = useState<NexusPanelMode>("monitor");
  ...
  const [exceptions, setExceptions] = useState<NexusException[]>([]);
  ...
  const policySummary = useMemo(() => {
    ...
    return `Nexus will bid up to $${cpm} CPM, shift up to $${deltaK}k/day autonomously, and ask for approval on decisions over $${gateK}k/day.`;
