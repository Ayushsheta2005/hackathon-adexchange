...
import type { CreativeRotation } from "./nexusMockCampaign.js";
import { CREATIVE_ROTATION_LABELS, MOCK_PMPS } from "./nexusMockCampaign.js";
import type { ParsedChip, ChipKey } from "./parseObjectiveChips.js";

export interface NexusCreateModeProps {
  step: 1 | 2 | 3;
  onStep: (s: 1 | 2 | 3) => void;
...
export function NexusCreateMode({
  step,
  onStep,
  objectiveText,
  onObjectiveText,
  ...
  onLaunch,
  onSaveDraft,
}: NexusCreateModeProps): JSX.Element {
    ...
    <div className="space-y-4">
      <div className="flex gap-1 rounded-lg border border-[oklch(0.91_0.005_80)] bg-[oklch(0.99_0.004_80)] p-1">
        {([1, 2, 3] as const).map((s) => (
          <button
            ...
            type="button"
            onClick={() => onStep(s)}
            className={`flex-1 rounded-md py-1.5 text-center font-nexus-mono text-[10.5px] font-semibold uppercase tracking-wide ${
              step === s
      ...
      {step === 1 ? (
        <div className="space-y-3">
          <label className="block">
            <span className="mb-1.5 block font-nexus-mono text-[9.5px] font-semibold uppercase tracking-wider text-[oklch(0.55_0.006_80)]">
              Objective
            </span>
            <textarea
              value={objectiveText}
              ...
              rows={5}
              placeholder="Tell Nexus what you want to achieve..."
              ...
              {parsedChips.map((c) => (
                <Fragment key={c.key}>
                  <button
                    ...
                    className={`rounded-full border px-2.5 py-1 font-nexus-mono text-[11px] font-medium ${
                      c.status === "confirmed"
        ...
        <div className="space-y-3">
            ...
            {ssps.map((s) => (
              ...
              >
                <div className="flex flex-col">
                  ...
                  {s.warnLowWinRate ? (
                    <span className="mt-0.5 font-nexus-mono text-[10px] font-medium text-amber-800">
          ...
          <div className="rounded-lg border border-[oklch(0.91_0.005_80)] bg-white">
            <button
              ...
              className="flex w-full items-center justify-between px-3 py-2.5 text-left font-nexus-mono text-[10px] font-semibold uppercase tracking-wider text-[oklch(0.55_0.006_80)]"
            >
              Deals / PMPs
              <span>{dealsOpen ? "−" : "+"}</span>
            </button>
            {dealsOpen ? (
              <ul className="list-none space-y-1 border-t border-[oklch(0.94_0.004_80)] p-3">
                {MOCK_PMPS.map((p) => (
                  <li
                    key={p.id}
                    className="font-nexus-mono text-[11.5px] text-[oklch(0.32_0.01_80)]"
                  >
                    {p.id} {p.label}{" "}
      ...
      {step === 3 ? (
        <div className="space-y-4">
          ...
          />
          <label className="block">
            <span className="mb-1 block font-nexus-mono text-[9.5px] font-semibold uppercase tracking-wider text-[oklch(0.55_0.006_80)]">
              Approval gate ($/day)
            </span>
            <input
              type="number"
              ...
              className="w-full rounded-lg border border-[oklch(0.91_0.005_80)] px-3 py-2 font-nexus-mono text-[13px]"
            />
          </label>
          <label className="block">
            <span className="mb-1 block font-nexus-mono text-[9.5px] font-semibold uppercase tracking-wider text-[oklch(0.55_0.006_80)]">
...
function LabeledSlider({
  ...
  return (
    <label className="block">
      <div className="mb-1 flex justify-between font-nexus-mono text-[9.5px] font-semibold uppercase tracking-wider text-[oklch(0.55_0.006_80)]">
        <span>{label}</span>
