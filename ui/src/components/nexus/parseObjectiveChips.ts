...
const EXAMPLE = "Maximize VCR for Nike 1P audience on CTV, $50k weekly cap, run through Sunday";
...
export function parseObjectiveToChips(text: string): ParsedChip[] {
  ...
  if (/nike|1p|audience|demo|household/i.test(t)) {
    chips.push({
      key: "audience",
      label: "Audience",
      value: /nike/i.test(t) ? "Nike 1P" : "CTV audience",
      status: /nike|1p/i.test(t) ? "confirmed" : "ambiguous",
    });
  } else {
