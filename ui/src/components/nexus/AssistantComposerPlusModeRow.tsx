...
import { IconPlus } from "./NexusComposerIcons.js";
...
export function AssistantComposerPlusModeRow({
  ...
  return (
    <div ref={wrapRef} className="nexus-composer-plus-wrap">
      <button
        type="button"
        className={`nexus-composer-plus-btn${open ? " nexus-composer-plus-btn--open" : ""}`}
        disabled={blocked}
        aria-expanded={open}
        aria-haspopup="menu"
      ...
      {open ? (
        <div
          ...
          className="nexus-composer-plus-menu"
        >
          {structured.map((m) => (
            <button
              key={m.id}
              type="button"
              role="menuitem"
              className="nexus-composer-plus-menu-item"
              onClick={() => pick(m.id)}
            >
              <span className="nexus-composer-plus-menu-icon" aria-hidden>
                {renderModeIcon(m.id)}
              </span>
              <span className="nexus-composer-plus-menu-label">{m.label}</span>
            </button>
          ))}
        </div>
      ) : null}
      {activeStructured ? (
        <div className={`nexus-composer-pill ${activeStructured.pillOnClassName}`}>
          <span aria-hidden>{renderModeIcon(activeStructured.id)}</span>
          <span>{activeStructured.label}</span>
          <button
            type="button"
            className="nexus-composer-attached-clear"
