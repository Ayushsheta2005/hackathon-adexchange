 ...
 */
export function createRealCircleSdk(config) {
...
export function buildAdapter(client) {
    return {
 ...
 */
export function normalizeUsdcAmount(raw) {
    const s = (raw ?? "").trim();
    if (!s)
        return "0.000000";
    ...
    if (fracRaw && !/^\d*$/.test(fracRaw))
        return `${whole || "0"}.000000`;
    const frac = (fracRaw + "000000").slice(0, 6);
    return `${whole || "0"}.${frac}`;
}
function toFixedFromScientific(s) {
    const n = Number(s);
...
//# sourceMappingURL=circleAdapter.js.map