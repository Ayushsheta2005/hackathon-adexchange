...
import { AdFormatSchema, AdInventoryListingSchema, AdSizeSchema, AdTypeSchema, UsdcAmountSchema, } from "@ade/shared";
import { z } from "zod";
/**
...
export function createListInventoryTool(deps) {
    const fetcher = deps.fetchImpl ?? fetch;
    const uuid = deps.randomUuidImpl ?? randomUUID;
    const now = deps.nowImpl ?? (() => new Date());
    return {
        name: "listInventory",
        description: "Register an ad inventory listing with the Exchange. Provide the ad type, format, size, floor price, and any contextual exclusions. Use when you have a new impression to sell.",
...
//# sourceMappingURL=listInventory.js.map