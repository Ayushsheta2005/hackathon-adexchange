...
const optionalAddr = blankToUndefined(z
    .string()
...
export function buildBuyerWalletRouting(config) {
    const m = new Map();
    const pairs = [
...
export function loadServerConfig(env = process.env) {
    ...
    if (!parsed.success) {
        throw new Error(`Invalid server env: ${parsed.error.issues
            .map((i) => `${i.path.join(".")} ${i.message}`)
            .join("; ")}`);
    }
    const data = parsed.data;
    const uiFixtureSeedEnabled = data.NODE_ENV !== "production" &&
        Boolean(data.UI_FIXTURE_SEED && ["1", "true", "yes"].includes(data.UI_FIXTURE_SEED.toLowerCase()));
