...
import { createGatewayAdapter } from "./middleware/nanopayments.js";
...
async function main() {
    ...
    if (config.uiFixtureSeedEnabled) {
        ...
        logger.warn({ replay: fixtureAuctionReplay.length }, "UI_FIXTURE_SEED enabled — in-memory demo data loaded; do not use in production");
    }
    else {
    ...
    const { app } = createApp({
        ...
        demo: {
            ...
            personas,
            gemini,
            buyerPrivateKey: config.BUYER_PRIVATE_KEY,
