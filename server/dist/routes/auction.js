...
export function createAuctionRouter(deps) {
    const router = Router();
    router.post("/auction/run/:listingId", createPauseGuard(deps.controlStore), createAuctionRateLimiter(5), async (req, res, next) => {
        try {
