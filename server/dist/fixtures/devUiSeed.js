...
export function buildFixtureListings() {
    return [
        {
...
export function buildFixtureBids() {
    const mk = (i, agent, wallet, amount) => ({
        bidId: BID_IDS[i - 1],
        buyerAgentId: agent,
        buyerWallet: wallet,
        targeting: {
...
export function buildFixtureAuctionReplay() {
    const rows = [
        {
    ...
    for (const r of rows) {
        ...
        if (!p.success)
            throw new Error(`fixture auction invalid: ${p.error.message}`);
    }
    return rows;
}
export function buildFixtureReceipts(auctions) {
    const out = auctions.map((a, i) => ({
    ...
    for (const r of out) {
        ...
        if (!p.success)
            throw new Error(`fixture receipt invalid: ${p.error.message}`);
    }
    return out;
}
/**
 ...
 */
export async function seedDevUiStores(input) {
    ...
    for (const L of listings)
        await input.listingStore.add(L);
    for (const b of bids)
        await input.bidStore.add(b);
    for (const r of receipts)
        await input.settlementStore.add(r);
    input.logger?.info({ listings: listings.length, bids: bids.length, receipts: receipts.length }, "ui_fixture_seed_stores_loaded");
}
//# sourceMappingURL=devUiSeed.js.map